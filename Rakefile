...
def all_links()
  ...
  end.sort.map{ |a| a.gsub(/^\//, 'http://www.tarunjangra.com/') }
end
...
SCSSLint::RakeTask.new do |t|
  f = Tempfile.new(['tarunjangra-', '.scss'])
...
task spell: [:build] do
  typos = all_html().reduce(0) do |total, f|
    ...
    tmp = Tempfile.new(['tarunjangra-', '.txt'])
...
task ping: [:build] do
  links = all_links().uniq
    .reject{ |a| a.start_with? 'http://www.tarunjangra.com/' }
  ...
  tmp.flush
  tmp.close
  out = Tempfile.new(['tarunjangra-', '.txt'])
  out.close
...
task orphans: [:build] do
  links = all_links()
    .reject{ |a| !a.start_with? 'http://www.tarunjangra.com/' }
    .map{ |a| a.gsub(/#.*/, '') }
  links += all_html().map { |f| f.gsub(/_site/, 'http://www.tarunjangra.com') }
  counts = {}
  links
