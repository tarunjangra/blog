...
Usage: send.rb [options]
  ...
  opt :dry, 'Dry run (always email to test@tarunjangra.com)'
...
if opts[:dry]
  emails = ['Tarun Jangra,test@tarunjangra.com']
else
...
emails.each do |line|
  ...
  mail = Mail.new do
    from 'Tarun Jangra <tarun@izap.in>'
    to address
    subject 'tarunjangra.com: ' + opts[:subject]
    message_id "<#{UUIDTools::UUID.random_create}@tarunjangra.com>"
    text_part do
